...
export async function streamCoachChat(payload, { onDelta, fetchImpl = fetch, signal } = {}) {
  ...
  // Server closed without a `done` event — the buffered text is possibly a
  // truncated reply. Return it flagged so the UI can say so instead of
  // presenting it as complete.
  return { configured: true, message: trimTrailer(full).trim(), question: null, offer_rethink: false, memory_note: null, incomplete: true };
}
