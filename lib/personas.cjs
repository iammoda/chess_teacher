// Coach persona definitions for server-side CommonJS code (Vercel's Node
// runtime may predate require(esm), so coach-chat.js loads this file).
//
// lib/personas.mjs is the browser copy of this catalog. The two files MUST
// stay identical — tests/personas-sync.test.mjs deep-compares them and fails
// the suite on any drift. Edit both files together.
//
...
const PERSONA_KEYS = Object.keys(COACH_PERSONAS);
