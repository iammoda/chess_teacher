...
function momentScore(move) {
  ...
  // Eval sign flip — the game changed hands on this move. evalBefore and
  // evalAfter are SIDE-TO-MOVE scores from opposite perspectives (before: the
  // player to move; after: the opponent), so the opponent's score must be
  // negated before comparing signs. Comparing them raw flagged every steady
  // advantage as a "flip" and missed the real reversals.
  if (Number.isFinite(move.evalBefore) && Number.isFinite(move.evalAfter)) {
    const playerBefore = move.evalBefore;
    const playerAfter = -move.evalAfter;
    if (
      Math.sign(playerBefore) !== Math.sign(playerAfter) &&
      Math.abs(playerAfter - playerBefore) >= 150
    ) {
      score += 200;
    }
  }
