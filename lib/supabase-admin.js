...
function createSupabaseAdmin(options) {
  ...
  async function selectForUser(table, userId, { limit = 10_000 } = {}) {
    const query = `${encodeURIComponent(table)}?select=*&user_id=eq.${encodeURIComponent(userId)}&limit=${limit}`;
