...
function validateChatPayload(payload) {
  ...
  // Optional context blocks must be shape-checked HERE: quota is consumed
  // before the fetch body is built, so a malformed entry that throws inside
  // buildChatInput would burn the user's allowance and misreport as a 502.
  if (payload.moment !== undefined && payload.moment !== null
    && (typeof payload.moment !== "object" || Array.isArray(payload.moment))) {
    return "Chat payload moment must be an object.";
  }
  if (payload.candidates !== undefined) {
    if (!Array.isArray(payload.candidates)) return "Chat payload candidates must be an array.";
    for (const candidate of payload.candidates) {
      if (!candidate || typeof candidate !== "object") return "Each candidate must be an object.";
    }
  }
  if (payload.weaknesses !== undefined) {
    if (!Array.isArray(payload.weaknesses)) return "Chat payload weaknesses must be an array.";
    for (const weakness of payload.weaknesses) {
      if (!weakness || typeof weakness !== "object") return "Each weakness must be an object.";
    }
  }
  if (payload.coachMemory !== undefined && payload.coachMemory !== null) {
    const memory = payload.coachMemory;
    if (typeof memory !== "object" || Array.isArray(memory)) return "Chat payload coachMemory must be an object.";
    if (memory.recentTraces !== undefined && memory.recentTraces !== null) {
      if (!Array.isArray(memory.recentTraces)) return "coachMemory.recentTraces must be an array.";
      for (const trace of memory.recentTraces) {
        if (!trace || typeof trace !== "object") return "Each coach memory trace must be an object.";
      }
    }
  }
  return null;
